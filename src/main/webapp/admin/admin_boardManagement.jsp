...
<body>
                ...
                <div class="row ">
                    <div class="col-3 admin_menu">
                        <table class="menuBar">
                            ...
                            <tr>
                                <p class="menu" id="adminMenu_user"><a href="/toUserManagement.admMem?currentPage=1" class="menu_link">회원
                                        관리</a></p>
                            </tr>
                            <tr>
                        ...
                        <div class="row admin_searchArea">
                            ...
                            <c:choose>
                                <c:when test="${empty value}">
                                    <form action="/searchProc.admBo?currentPage=1" method="post" id="searchForm">
                                        <div class="col admin_search" id="admin_search_Area">
                                            <select name="search_category" id="search_category">
                                                <option value="post_no">글번호</option>
                                ...
                                <c:otherwise>
                                    <form action="/searchProc.admBo?currentPage=1" method="post" id="searchForm">
                                        <div class="col admin_search" id="admin_search_Area">
                                            <select name="search_category" id="search_category">
                                                <option value="post_no" <c:if
                                                    test="${search_category eq 'post_no'}">
                                                    selected='selected'
                                                    </c:if> >글번호</option>
                                                <option value="idOrNickname" <c:if test="${search_category eq 'idOrNickname'}">
                                                    selected='selected'</c:if>>아이디/닉네임</option>
                                                <option value="titleOrContent" <c:if test="${search_category eq 'titleOrContent'}">
                                    ...
                                    <tbody>
                                        <c:forEach var="dto" items="${list}">
                                            <tr>
                                                ...
                                                <td><a href="/toDetailPostByAdmin.admBo?post_no=${dto.post_no}" class="userDataLink">
                                                        <c:out value="${dto.post_no}" />
                                                    </a></td>
                                                <td><a href="/toDetailPostByAdmin.admBo?post_no=${dto.post_no}" class="userDataLink">
                                                        <c:out value="${dto.post_title}" />
                                                    </a></td>
                                                <td><a href="/toDetailPostByAdmin.admBo?post_no=${dto.post_no}" class="userDataLink">
                                                        <c:out value="${dto.post_writer_nickname}" />
                                                    </a></td>
                                                <td><a href="/toDetailPostByAdmin.admBo?post_no=${dto.post_no}" class="userDataLink">
                                                        <c:out value="${dto.post_createdDate}" />
                                                    </a></td>
                                                <td><a href="/toDetailPostByAdmin.admBo?post_no=${dto.post_no}" class="userDataLink">
                                                        <c:out value="${dto.post_view_count}" />
                                                    </a></td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                                        ...
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                ...
                                                <c:forEach var="i" begin="${naviMap.get('startNavi')}"
                                                    ...
                                                    <li class="page-item"><a class="page-link"
                                                            href="/searchProc.admBo?currentPage=${i}&search_category=${search_category}&search_input=${value}">${i}</a>
                                                    </li>
                                                </c:forEach>
                                                <c:if test="${naviMap.get('needNext') eq true}">
                                                    <li class="page-item">
                                                        <a class="page-link"
                                                            href="/searchProc.admBo?currentPage=${naviMap.get('endNavi')+1}&search_category=${search_category}&search_input=${value}"
                                                            aria-label="Next">
                                                            <span aria-hidden="true">&raquo;</span>
                                                        </a>
                                                    </li>
    ...
    <script>
        ...
        $("#btnDelete").click(function(){
        	let post_no = test();
        	console.log(post_no);
        	if(confirm("글번호 : "+post_no+ " 게시물을 삭제처리 하시겠습니까?")){
        		location.href = "/toDeleteProc.admBo?post_no="+post_no;
        	}
        })
        ...
        $("#search_button").click(function () {
            ...
            if (($('#search_input').val()) == "") {
                ...
                return;
            }else if (($('#search_category').val()) == "post_no") {
